...
public class OrderController {
    ...
    public ResponseEntity<OrderResponseDto> createOrder(@RequestBody OrderRequestDto orderRequestDto) {
        ...
        if (orderResponseDto.getMessage().equals("Order saved successfully")) {
            return ResponseEntity.status(HttpStatus.OK).body(orderService.createOrder(orderRequestDto));
        } else  {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(orderService.createOrder(orderRequestDto));
        }
    }
}
